...
# Récupérer la fenêtre actuellement en focus
    ...
    hwnd_target = find_mgba_window(WINDOW_TITLE)
    ...
    # Rendre le focus à la fenêtre originale
    if original_hwnd:
        win32gui.SetForegroundWindow(original_hwnd)
        print("[INFO] Focus rendu au process original :", original_hwnd)
    else:
        print("[WARNING] No original process found.")

    ctypes.windll.user32.AttachThreadInput(fore_thread, target_thread, False)
    print("[INFO] Savestate loaded.")
